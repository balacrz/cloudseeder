 ...
 *  + OPTIONAL: expr  (JS expression evaluated with rec, ctx)
 */
...
// ---- NEW: tiny, sandbox-lite expr runner ----
function buildExprRunner(expr) {
  // Evaluate with 'rec' and 'ctx' available.
  // NOTE: This is sandbox-lite. If filter specs can be user-supplied,
  // consider hardening or whitelisting.
...
export function matchPredicate(rec, spec, ctx = {}) {
  ...
  if (spec.expr) {
    try {
      ...
      if (ctx?.env?.DEBUG_FILTERS) {
        console.warn(`[filter.expr] Failed: ${spec.expr}`, e?.message);
      }
      return false;
    }
  }
